...
| Source | How |
|---|---|
| Recent Discord messages | Fetches last 100 messages from all channels the bot has participated in |
| Stored chat summaries | Reads daily summaries from `.bot-history/` (generated by the summarizer) |
...
The bot fetches up to `REMEMBER_MAX_MESSAGES` messages (default: 100) from up to `REMEMBER_MAX_CHANNELS` channels (default: 10), plus any stored summaries from the summarizer. Results include citations — Discord message timestamps and file paths — so you can trace back to the source.

`/remember` always uses a fresh session so it doesn't pollute your ongoing conversation.

### Chat history summarization

The `/remember` command only sees the last 100 messages per channel. To recall older conversations, run the summarizer to compress chat history into searchable daily summaries:
